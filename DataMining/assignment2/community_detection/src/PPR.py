...
def sweepCut(G, ppr, local_minima=float('inf')):
    ...
    for node in tqdm(sorted_nodes):
        ...
        if len(S) == 0:
            S.append(node)
            _conductance, cut_size, volume = conductance(G, S, m)
...
def conductance(G, S, m):
    ...
    _conductance = cut_size / volume
    return _conductance, cut_size, volume, 
...
def main():
    ...
    conductance_list= sweepCut(G, ppr)

    ## Find local minima in the conductance_list
    local_minima = find_local_minima(conductance_list)
    best_set = sweepCut(G, ppr, local_minima=random.choice(local_minima))
