...
def conductance_gain(G, S, node, old_cut_size, old_volume, m):
    ...
    _conductance_gain = (old_cut_size / old_volume) - (new_cut_size / new_volume)

    return _conductance_gain, new_cut_size, new_volume
...
def sweepCut(G, ppr):
    ...
    for node in tqdm(sorted_nodes):
        ...
        S.append(node) 
        gain, cut_size, volume = conductance_gain(G, S, node, cut_size, volume, m)
    
        current_conductance = cut_size / volume
        conductance_list.append(current_conductance)
