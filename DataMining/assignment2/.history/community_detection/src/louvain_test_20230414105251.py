...
def modularity(G, communities, m):
    ...
    for community in communities:
        for node in community:
            # Compute the sum of the weights of the edges between node and its neighbors in the same community
            intracommunity_weight = sum(G[node][neighbor]['weight'] for neighbor in G.neighbors(node) if neighbor in community)
            
            # Compute the sum of the weights of the edges between node and all of its neighbors
            degree = sum(G[node][neighbor]['weight'] for neighbor in G.neighbors(node))
            
            # Compute the sum of the weights of the edges between node and its neighbors in different communities
            intercommunity_weight = degree - intracommunity_weight
            
            # Update Q
            Q += intracommunity_weight/m - (intercommunity_weight/(2*m))**2
