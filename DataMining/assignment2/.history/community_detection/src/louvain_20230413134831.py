...
def partitioning(G, partition):
    ...
    not_converge = True
    m = sum(weight for _, _, weight in G.edges(data='weight').values() if weight is not None)
    print(m)
    while not_converge:
        ...
        for comm in nodes:
            if comm != max_comm1 and comm != max_comm2:
                ...
                if contracted_G.has_edge(max_comm1, comm):
                    new_weight_out += contracted_G[max_comm1][comm]['weight']
                if contracted_G.has_edge(max_comm2, comm):
                    new_weight_out += contracted_G[max_comm2][comm]['weight']
                if contracted_G.has_edge(comm, max_comm1):
                    new_weight_in += contracted_G[comm][max_comm1]['weight']
                if contracted_G.has_edge(comm, max_comm2):
                    new_weight_in += contracted_G[comm][max_comm2]['weight']
...
def main():
    ...
    while True:
        ...
        if contracted_G == G and new_partition == partition:
            break
        partition = new_partition
        G = contracted_G
