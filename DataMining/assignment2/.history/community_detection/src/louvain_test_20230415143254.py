...
def second_stage(G, communities):
    ...
    while changed:
        ...
        # get the communities of the supernodes using the first stage of the Louvain method
        supernode_communities = first_stage(supernode_graph)
        
        # update the node to supernode mapping based on the new community assignments
        new_node_to_supernode = {}
        for supernode, supernode_community in supernode_communities.items():
            for node in supernode_to_partition[supernode]:
                new_node_to_supernode[node] = supernode_community
        
        # check if the community configuration has changed
        changed = (new_node_to_supernode != node_to_supernode)
        node_to_supernode = new_node_to_supernode
    
    # return the final community assignments for the nodes
    return node_to_supernode
...
def get_modularity_gain(G, node, com_neighbor, communities):
    ...
    for neighbor in G.neighbors(node):
        if communities[neighbor] == com_neighbor:
            k_i_in += 1
            w_i_in += G[node][neighbor]['weight']
        k_i_tot += 1
        w_i_tot += G[node][neighbor]['weight']
    k_i_out = k_i_tot - k_i_in
    w_i_out = w_i_tot - w_i_in
    sigma_tot = sum(G.degree(weight='weight'))
    ...
    for neighbor in G.neighbors(node):
        if communities[neighbor] == com_neighbor:
            ...
            w_c += G[node][neighbor]['weight']
    m_c = sigma_c / (2 * sigma_tot)
    return (2 * w_i_in - k_i_tot * m_c * w_i_tot / sigma_tot) - (2 * w_i_tot - k_i_tot * sigma_in * m_c / sigma_tot) * (2 * m_c)
