...
def restructuring(G, partition):
    ...
    while True:
        ...
        for comm in nodes:
            if comm != max_comm1 and comm != max_comm2:
                contracted_G.add_edge(new_comm, comm, weight=0)
                for node, node_comm in partition.items():
                    if node_comm == comm:
                        if G.has_edge(node, node2):
                            contracted_G[new_comm][comm]['weight'] += G[node][node2]['weight']
                        if G.has_edge(node, node1):
                            contracted_G[new_comm][comm]['weight'] += G[node][node1]['weight']
    ...
    return partition, contracted_G3
