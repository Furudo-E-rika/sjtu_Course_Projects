...
def partition(G, partition):
    # initialize each node to be in its own community
    partition = {i: i for i in G.nodes()}
    ...
    while merged:
        ...
        for node in nodes:
            ...
            for neighbor in G.neighbors(node):
                ...
                delta_Q = (in_weight + G[node][neighbor]['weight']) / G.size(weight='weight') - \
                          (tot_weight + G.degree(node, weight='weight')) ** 2 / (4 * G.size(weight='weight') ** 2) - \
                          (in_weight ** 2) / (4 * G.size(weight='weight') ** 2) + \
                          (tot_weight ** 2) / (4 * G.size(weight='weight') ** 2) + \
                          (G.degree(node, weight='weight') ** 2) / (4 * G.size(weight='weight') ** 2)
