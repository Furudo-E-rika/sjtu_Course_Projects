...
def _node2comm(G):
    ...
    for i, node in enumerate(G.nodes()):
        node2comm[node] = i  ## set each node itself as a community
        for edge in G[node].items():
            edge_weights[node][edge[0]] = edge[1].get('weight', 1)
    return node2comm, edge_weights

def partitioning(G, node2com, edge_weights):
    
    not_converge = False
    all_weights = sum[weight for start in edge_weights.keys() for end, weight in edge_weights[start].items()] /2
