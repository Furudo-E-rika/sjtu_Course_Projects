...
def compute_modularity(G, partition):
    ...
    G : networkx.Graph
        The input graph.
    partition : dict
        A dictionary that maps each node to its community.
    ...
    for comm in set(partition.values()):
        nodes = [n for n in partition.keys() if partition[n] == comm]
        subgraph = G.subgraph(nodes)

        # Compute the fraction of edges within the community
        L_c = sum(subgraph[u][v].get('weight', 1) for u, v in subgraph.edges)
        d_c = sum(G.degree(n, weight='weight') for n in nodes)
        Q += (L_c / m) - ((d_c / (2*m))**2)
...
def main():
    ...
    while True:
        old_modularity = modularity(G, partition)
        partition = partitioning(G, partition)
        contracted_G, new_partition = restructuring(G, partition)
        new_modularity = modularity(contracted_G, new_partition)
        print(old_modularity, new_partition)
