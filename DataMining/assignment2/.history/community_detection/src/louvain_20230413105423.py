...
def phase1(G, partition):
    while True:
        found = False
        for i in G.nodes():
            ci = partition[i]
            ki_in = sum([G.has_edge(i,j) for j in G.neighbors(i)])
            ci_size = len(partition[ci])
            max_delta_Q = 0
            max_cj = ci
            for j in G.neighbors(i):
                cj = partition[j]
                kj_in = sum([G.has_edge(j,k) for k in G.neighbors(j)])
                cj_size = len(partition[cj])
                delta_Q = (kj_in - ki_in) / (2 * G.number_of_edges()) + (ci_size * cj_size) / (2 * G.number_of_edges())**2 - (ci_size * ci_size) / (2 * G.number_of_edges())**2 - (cj_size * cj_size) / (2 * G.number_of_edges())**2
