    ...
    for epoch in range(config['nb_epoch']):
        ...
        for batch in data_loader:
            ...
            if global_step % args.log_every == 0:
                ...
                logger.info(f'{timestamp} epo:[%d/%d] itr:[%d/%d] step_time:%ds Loss=%.5f' %
            ...
            if global_step % args.valid_every == 0:
                ...
                valid_result = validate(valid_set, model, 1000, TOPK)
...
##### Evaluation #####
def validate(valid_set, model, pool_size, TOPK):
    """
    ...
    data_loader = torch.utils.data.DataLoader(dataset=valid_set, batch_size=64,
    ...
    for k in tqdm(range(0, n_processed, pool_size)):
        ...
        for i in range(len(desc_pool)):  # for i in range(pool_size):
