...
for i in range(0, numDocs):
  ...
  for shingle in shingle_list:
        ## cause that we want to convert the digest to a 32-bit integer
        ## we only take the first 4 elements of the digest
        hash_shingle = sha1(shingle.encode('utf-8')).digest()[:4]
        hash_int = int.from_bytes(hash_shingle, byteorder='big')
        shinglesInDoc.add(hash_int)
...
# sample (fake) 
estJSim = [0 for _ in range(int(numDocs * (numDocs - 1) / 2))] 
def getTriangleIndex(i,j):
    ## note that our index starts from zero, so the statement will be slightly different from the stanford version
    assert i != j
    if i > j:
        i, j = j, i
...
for docID in docNames:
  ...
  for i in range(numHashes):
    min_hash = nextPrime + 1
    for hash_int in shingleIDSet:
      hash_code = (a_list[i] * hash_int + b_list[i]) % nextPrime
...
for i in range(0, numDocs):
  ...
  for j in range(i + 1, numDocs):
    ...
    estJSim[getTriangleIndex(i, j)] = intersection / numHashes
...
for i in range(0, numDocs):  
  for j in range(i + 1, numDocs):
    ...
    if estJ > threshold:
      ...
      set_i = docsAsShingleSets[docNames[i]]
      set_j = docsAsShingleSets[docNames[j]]

      J = len(set_i.intersection(set_j)) / len(set_i.union(set_j))
