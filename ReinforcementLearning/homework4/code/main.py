...
def learn(batch_size, current_model, target_model, replay_buffer, optimizer, step=1):
    ...
    if USE_DOUBLE_Q:
        ...
        next_action = torch.max(next_q_values, 1)[1]
        next_q_value = next_q_values_target.gather(1, next_action.unsqueeze(1)).squeeze(1)
    ...
    loss = torch.mean(F.mse_loss(expected_q_value, q_value))
    """ ------------- Programming 4 ------------- """
...
if __name__ == '__main__':
    ...
    ENV_NAME = "Pendulum-v0"  ## origin version: Pendulum-v0
    set_env(ENV_NAME)

    num_frames = 30000 ## original value: 30000
    epsilon_start = 0.5
    epsilon_final = 0.01
    epsilon_decay = 1500
