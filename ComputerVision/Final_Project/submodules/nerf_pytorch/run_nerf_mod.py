...
def raw2outputs(raw, z_vals, rays_d, sdf, raw_noise_std=0, white_bkgd=False, pytest=False):
    """ A helper function for `render_rays`.
    """
    density_func = LaplaceDensity()
    density_flat = density_func(sdf)
...
def compute_sdf(x, sdf_bounding_sphere=3.0, sphere_scale=1.0):
    #x.requires_grad_(True)
    sigma = MLP(x.shape[1], 256, 257)
    output = sigma(x)
    sdf = output[:,:1]

    if sdf_bounding_sphere > 0.0:
        sphere_sdf = sphere_scale * (sdf_bounding_sphere - x.norm(2,1, keepdim=True))
        sdf = torch.minimum(sdf, sphere_sdf)
    
    feature_vectors = output[:, 1:]

    return sdf, feature_vectors
...
def render_rays(ray_batch,
    ...
    sdf, feature_vectors = compute_sdf(pts_flat)

#     raw = run_network(pts)
    raw = network_query_fn(pts, viewdirs, network_fn, features)
    rgb_map, disp_map, acc_map, weights, depth_map = raw2outputs(raw, z_vals, rays_d, sdf, raw_noise_std, white_bkgd, pytest=pytest)
