...
for i in tqdm(range(data.train_images.shape[0])):
    ...
    for (kp, des) in zip(kp_vector, des_vector):
        vec_dict[data.train_lb[i]]['kp'].append(kp)
        vec_dict[data.train_lb[i]]['des'].append(des)
...
for i in range(1, 102):
    vec_list += vec_dict[i]['des'][0:bneck_value]
...
N_clusters = 3000
kmeans = KMeans(n_clusters=N_clusters, max_iter=1000).fit(vec_list)
...
for i in tqdm(range(num_images)):
    ...
    for (kp, des) in zip(kp_vector, des_vector):
        ...
        hist_vector[i][cluster] += 1
    hist_vector[i] = (hist_vector[i] - np.min(hist_vector[i])) / (np.max(hist_vector[i]) - np.min(hist_vector[i]))
...
for i in tqdm(range(num_test_images)):
    ...
    for (kp, des) in zip(kp_vector, des_vector):
        cluster = kmeans.predict([des])
        hist_test_vector[i][cluster] += 1
    hist_test_vector[i] = (hist_test_vector[i] - np.min(hist_test_vector[i])) / (np.max(hist_test_vector[i]) - np.min(hist_test_vector[i]))
