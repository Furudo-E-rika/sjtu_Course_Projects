...
class SLUTagging(nn.Module):
    ...
    def forward(self, batch):
        ...
        tokens = [self.bert_tokenizer.tokenize(x) for x in utt]
        input_ids = [self.bert_tokenizer.convert_tokens_to_ids(["[CLS]"] + token + ["[SEP]"]) for token in tokens]
        input_ids = [x + [self.bert_tokenizer.pad_token_id] * (MAX_LENGTH-len(x)) for x in input_ids]
        input_ids = torch.cat(torch.tensor(x) for x in input_ids)
        ...
        for i in range(len(lengths)):
            ...
            tag_mask = [0] + tag_ids_list[i] + [0] * (MAX_LENGTH - len(tag_ids_list[i]) - 1)
            tag_ids.append(tag_id)
            tag_masks.append(tag_mask)

        input_ids = torch.stack([torch.tensor(x) for x in input_ids])
        input_mask = torch.stack([torch.tensor(x) for x in input_mask])
        tag_ids = torch.stack([torch.tensor(x) for x in tag_ids])
        tag_masks = torch.stack([torch.tensor(x) for x in tag_masks])


        bert_output = self.bert_model(input_ids, attention_mask=input_mask)

        hiddens = self.dropout_layer(bert_output)
